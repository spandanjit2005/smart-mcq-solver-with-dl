...
CHUNK_EMBEDDING_MODEL = "Qwen/Qwen3-Embedding-0.6B"
VECTOR_EMBEDDING_MODEL = "Qwen/Qwen3-Embedding-4B"

ARXIV_PATH = "rag_pipeline/data/data_corpus/arxiv_corpus.parquet"
PUBMED_PATH = "rag_pipeline/data/data_corpus/pubmed_corpus.parquet"
WIKIPEDIA_PATH = "rag_pipeline/data/data_corpus/wikipedia_corpus.parquet"

CHUNKED_DATA_JSONL = "rag_pipeline/data/data_chunks/knowledge_chunks.jsonl"
CHUNKED_DATA_PARQUET = "rag_pipeline/data/data_chunks/knowledge_chunks.parquet"
...
tokenizer = AutoTokenizer.from_pretrained(VECTOR_EMBEDDING_MODEL)
...
arxiv_data = pl.read_parquet(ARXIV_PATH)

for row in arxiv_data.iter_rows(named=True):
    text_to_chunk = f"{row["title"]}. {row["summary"]}"
...
pubmed_data = pl.read_parquet(PUBMED_PATH)
...
wiki_data = pl.read_parquet(WIKIPEDIA_PATH)

for row in wiki_data.iter_rows(named=True):
    clean_full_text = clean_wikipedia_text(row["full_text"])
    text_to_chunk = f"{row['title']}. {row['summary']} {clean_full_text}"
...
data_chunks_df.write_ndjson(CHUNKED_DATA_JSONL)
data_chunks_df.write_parquet(CHUNKED_DATA_PARQUET, compression="zstd")

logger.info(f"Saved {len(data_chunks)} chunks to disk as jsonl and parquet files.")